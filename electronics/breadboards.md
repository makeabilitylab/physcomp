...
That said, breadboards can be a bit confusing at first and do take time to get used to. In this lesson, we'll learn about breadboards, prototype with them in the simulation environment called [Tinkercad Circuits](https://www.tinkercad.com/circuits), and build some simple circuits with resistors and LEDs.

## Inside a breadboard

If you tore open a breadboard, you would see a set of *pre-wired* columns and rows (internal metal strips). These pre-wired connections are exposed as holes on the top of the breadboard (arranged in a grid), which you can quickly and easily tap into to make circuits.

Unfortunately, it is not immediately clear *which* holes are pre-wired to one another—which introduces a lot of confusion and miswirings for novices. So, it's important that you study and understand these breadboarding images (and also be patient with yourself as you're learning—a very common and frustrating mistake for beginners is an 'off-by-one' error or misunderstanding how the embedded wiring in a breadboard works).

![Cross-section view of a breadboard showing the internal metal strips that connect holes together, with the plastic cover partially removed to reveal the spring clips underneath](assets/images/InsideABreadboard.png)
...
![A circuit schematic showing a 9V battery connected to a resistor and red LED in series](assets/images/SimpleBreadboardExample_BuildLEDCircuit_ByJonFroehlich.png)
...
Let's build our first breadboard circuit. In this version, we're trying to keep things very clear. Consequently, we're using more jumper wires than necessary. Let's get started.
...
{: .warning }
> In general, do **not** connect your power source until you've finished building your circuit. Breadboarding with a live source is a common beginner mistake that can damage components.

![Step 1 of breadboarding: a 9V battery connected to the breadboard power rails, with the positive terminal wired to the red (+) rail and negative terminal to the black (-) rail](assets/images/BreadboardingLEDCircuit_Step1_ByJonFroehlich.png)
...
![Step 2 of breadboarding: a resistor placed with one leg in the positive power rail and the other in a column, creating Node C](assets/images/BreadboardingLEDCircuit_Step2_ByJonFroehlich.png)

#### Step 3: Add the LED

Third, let's add in the LED. We can place it anywhere we want but let's put it close to the resistor (since we know we'll have to connect them). Importantly, we cannot put the LED in such that both legs are in the same column—this would short the LED since both legs would be electrically connected.

![An incorrect LED placement showing both legs in the same column, with a red X indicating this is wrong because both legs would be electrically connected](assets/images/BreadboardingLEDCircuit_Step3-DontDoThis_ByJonFroehlich.png)
...
![Correct LED placement with each leg in a different column, spanning two separate nodes](assets/images/BreadboardingLEDCircuit_Step3_ByJonFroehlich.png)

#### Step 4: Connect LED anode (+) leg

Now, let's wire up the LED to our circuit starting with the LED anode (`+`) leg first (which must point towards the higher electric potential, *i.e.,* towards the resistor and positive rail in our circuit).

![Step 4: a jumper wire connecting the LED's anode column to the resistor's column, creating an electrical connection between the resistor and LED](assets/images/BreadboardingLEDCircuit_Step4_ByJonFroehlich.png)

#### Step 5: Connect LED cathode (-) leg

Fifth, let's wire up the LED cathode leg. In this case, we'll create an intermediate wire connection to a different point of our breadboard just to keep things clean. However, we could just place a jumper wire from the cathode leg directly to the `GND` (`-`) power rail.

![Step 5: a jumper wire from the LED's cathode column to another column, extending the ground-side node](assets/images/BreadboardingLEDCircuit_Step5_ByJonFroehlich.png)
...
![Step 6: a jumper wire connecting the intermediate node to the ground power rail, completing the circuit](assets/images/BreadboardingLEDCircuit_Step6_ByJonFroehlich.png)
...
![Three views of the same LED circuit: the physical breadboard layout on the left, a schematic diagram overlaid on the breadboard in the center, and arrows showing conventional current flow from positive terminal through the resistor, through the LED, and back to the negative terminal on the right](assets/images/BreadboardExampleWithUnderlyingCircuitSchematicAndCurrentFlow_ByJonFroehlich.png)
...
![Variation 2 of the LED circuit using power rails on both sides of the breadboard](assets/images/BreadboardExample2_UsingPowerRailsOnBothSides.png)

### Variation 3
Again, the same circuit but breadboarded differently. This version uses no jumper wires and simply connects the resistor and LED directly to the power rails.

![Variation 3 of the LED circuit with no jumper wires, where the resistor and LED are placed directly between the power rails and component columns](assets/images/BreadboardExample3_NoExternalWires.png)

## Some breadboard tips

### Tip 1: Use power rails

Connect your voltage source to the power rails first, then tap into them as needed rather than running separate wires from the source for each component.

![Diagram showing best practice of connecting components to the power rails rather than running long wires directly to the power source](assets/images/BreadboardTips_UsePowerRails.png)

### Tip 2: Link power rails across board

On larger breadboards, the power rails on the left and right sides may not be connected to each other. Use jumper wires to link them if you need power on both sides.

![Diagram showing how to connect the power rails on the left side of the breadboard to the power rails on the right side using jumper wires](assets/images/BreadboardTips_LinkPowerRails.png)

### Tip 3: Use wire colors to convey meaning

Use red wires for positive voltage, black wires for ground, and other colors for signal connections. This makes debugging much easier.
...
To learn more about breadboards and gain experience with them before building physical circuits, let's use [Tinkercad Circuits](https://www.tinkercad.com/) to play around. Tinkercad makes it easy to rapidly prototype, build, and simulate circuits in a software environment. And for those who do not have access to electronic tools, Tinkercad also provides a simple multimeter and oscilloscope—so we can test and measure our circuits too!

One particularly useful feature for learning about breadboards is that Tinkercad auto-highlights connected nodes when you mouse over breadboard columns and rows, which helps build understanding before using a physical breadboard.
...
![Screenshot of Tinkercad's registration page showing options for personal and school accounts](../arduino/assets/images/Tinkercad_RegistrationPage.png)

### Step 2: Create a new circuit

In your dashboard, click on "Circuits":

![Screenshot of Tinkercad's dashboard with the Circuits tab highlighted](../arduino/assets/images/Tinkercad_ClickOnCircuits.png)

Then click on "Create new Circuit":

![Screenshot of Tinkercad's dashboard showing the Create new Circuit button](../arduino/assets/images/Tinkercad_CreateNewCircuit.png)

### Step 3: Acquaint yourself with Tinkercad
...
![Screenshot of the Tinkercad Circuits interface showing the component sidebar on the right, a breadboard with a simple LED circuit on the canvas, and the Start Simulation button at the top](assets/images/Tinkercad_ShowingInterfaceWithBasicLEDCircuitOnBreadboard.png)

### Step 4: You can even embed created circuits onto webpages

Tinkercad also has a cool feature to embed working versions of your circuits onto webpages. Here's a simple LED circuit with a 9V battery supply, a 470Ω resistor, and an LED. You can open [this circuit](https://www.tinkercad.com/things/9FSld5pOJbd) in Tinkercad yourself and start to remix it as well!
...
**Embedded Circuit.** We've embedded a simple LED circuit made in [Tinkercad](https://www.tinkercad.com/things/9FSld5pOJbd). You can open it [here](https://www.tinkercad.com/things/9FSld5pOJbd) and play with it and/or build on it.
{: .fs-1 }
...
- [Setting Up a Breadboard](https://itp.nyu.edu/physcomp/Labs/Breadboard/), NYU ITP
