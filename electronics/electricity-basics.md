...
**Figure.** *Current* is the flow of charged particles (in this case, electrons) through a conductor. In the animation above, we are illustrating "current flow" as a dotted green line, which flows from the positive terminal of the 9V battery, through an LED and resistor, and then back to the 9V battery to its negative terminal. Animation from [The Engineering Mindset](https://youtu.be/kcL2_D33k3o).
{: .fs-1 }

*Current* is the flow of charged particles (in this case, electrons) through a conductor. Electric current is similar to water current moving through a pipe. To measure the flow of water, we could count the number of water molecules flowing past a given cross-section of pipe in time $$t$$. In fact, electric current $$I$$ is defined as the amount of charge $$Q$$ moving through a cross-section of wire in time $$t$$:

$$I = \frac{\Delta{Q}}{\Delta{t}}$$

Importantly, just like your home plumbing system, where water flows instantly out of your tap when you open the valve (propelled, for example, by water pressure from a water tower), so too does current flow instantly when a voltage is applied (propelled, for example, by a battery). And, critically, the water molecules that touch your hand did not flow all the way through your plumbing system in an instant. Instead, at all times, your pipes are completely filled with pressurized water. When you open the tap, the water molecules that touch your hands were the molecules pushing against that tap's valve (sort of like a first-in, first-out queue). This is similar to current in a circuit—atoms are tightly packed in a material with orbiting electrons. When a voltage is applied, these electrons start to "hop" from one atom to another through a conductor.
...
The SI unit of electric current is the *ampere* or *amp* (A), which is the flow of electric charge in coloumbs per second ($$C/s$$). A [couloumb (C)](https://en.wikipedia.org/wiki/Coulomb) is the SI unit for electric charge and is approximately $$6.24 × 10^{18}$$ electrons. Thus:
...
In circuits, we use *conventional current* to model the flow of electrons from positive to negative; however, electrons are actually moving in the *opposite* direction (called *electron flow*). This causes great confusion to novice makers.
...
Rest assured, all circuit diagrams use *conventional current* (modeling flow of charge from positive to negative). Thankfully, it turns out that as long as you're consistent, it does not matter. The math will still work out and even mnemonics like the [right-hand rule](https://en.wikipedia.org/wiki/Right-hand_rule) are based on conventional current (point thumb in direction of current $$I$$, see direction of electric field $$B$$).
